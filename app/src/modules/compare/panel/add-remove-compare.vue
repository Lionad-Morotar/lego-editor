<template>
  <div class="segment">
    <div class="content">
      <el-button class="action-button" :disabled="v===max" type="text" @click="add"
        >增加对比</el-button
      >
      <el-button class="action-button" :disabled="v===min" type="text" @click="subtract"
        >减少对比</el-button
      >
    </div>
  </div>
  ...
  methods: {
    ...
    subtract () {
      if (this.v !== this.min) {
        this.v -= 1
        this.$nextTick(() => {
          this.props.compares.splice(
            this.props.compares.length - 1, 1
          )
        })
      }
    }
